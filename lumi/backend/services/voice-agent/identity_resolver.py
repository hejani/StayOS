"""Identity resolution module for the StayOS Voice Agent on AgentCore.

Validates Cognito Access Tokens by calling the Cognito GetUser API and extracts
custom:propertyId and custom:gmAlias for property-scoped data access. Replaces
the previous PyJWT-based auth.py module.

Role in project: Called by server.py after the first WebSocket message is received.
The Access Token (sent by the browser as the first message) is validated via
Cognito GetUser, which simultaneously verifies token validity and retrieves
the user's custom attributes. The extracted propertyId scopes all subsequent
DynamoDB tool queries to the GM's property.
...
async def resolve_identity(access_token: str) -> Dict[str, str]:
    ...
    # Extract custom:propertyId - required for property-scoped DynamoDB access.
    property_id = _extract_attribute(user_attributes, "custom:propertyId")

    # Extract custom:gmAlias - required for settings lookup and logging context.
